...
//     // Virtual Threads do not have Names.
//     threadFactory = Thread.ofVirtual().name(Thread.currentThread().getName() + "-virtual-", 1).factory();
// }
// else {
//     threadFactory = Thread.ofVirtual().factory();
// }
//
// final Set<String> versions = new TreeSet<>(Comparator.reverseOrder());
//
// // Executors.newVirtualThreadPerTaskExecutor();
// try (ExecutorService executorService = Executors.newThreadPerTaskExecutor(threadFactory)) {
//     final CompletionService<List<String>> completionService = new ExecutorCompletionService<>(executorService);
//
//     for (URI repository : repositories) {
//         final CheckedSupplier<List<String>> checkedSupplier = () -> doQuery(repository, groupId, artifactId);
//         final Callable<List<String>> callable = () -> failsafeExecutor.get(checkedSupplier);
//         completionService.submit(callable);
//     }
//
//     for (int i = 0; i < repositories.size(); i++) {
//         try {
//             final List<String> list = completionService.take().get();
//
//             if (list != null) {
