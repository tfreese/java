...
public class ProcessMonitorPainter extends AbstractMonitorPainter {
    ...
    public double paintValue(final GraphicsContext gc, final double width) {
        ...
        for (final ProcessInfo processInfo : infos.getSortedByCpuUsage(3)) {
            y += fontSize * 1.25D;

            paintText(gc, processInfo.name(), x, y);
            paintText(gc, String.format("%8d %8.2f %8.2f", processInfo.pid(), processInfo.cpuUsage() * 100D, processInfo.memoryUsage() * 100D), x + 100D, y);
        }
        ...
        for (final ProcessInfo processInfo : infos.getSortedByMemoryUsage(3)) {
            y += fontSize * 1.25D;

            paintText(gc, processInfo.name(), x, y);
            paintText(gc, String.format("%8d %8.2f %8.2f", processInfo.pid(), processInfo.memoryUsage() * 100D, processInfo.cpuUsage() * 100D), x + 100D, y);
        }
