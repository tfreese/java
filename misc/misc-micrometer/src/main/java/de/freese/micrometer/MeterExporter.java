...
public final class MeterExporter {
    ...
    public static List<String> export(MeterRegistry registry, final Duration step, TimeUnit baseTimeUnit) {
        ...
        registry.getMeters().stream().sorted(comparator).forEach(m -> m.use(gauge -> {
            ...
            String text = id(timer) + ": throughput=" + unitlessRate(step, count) + "; mean=" + time(baseTimeUnit, snapshot.mean(baseTimeUnit)) + "; max=" + time(baseTimeUnit, snapshot.max(baseTimeUnit));
            list.add(text);
        }, summary -> {
            ...
            String text = id(summary) + ": throughput=" + unitlessRate(step, count) + "; mean=" + value(summary, snapshot.mean()) + "; max=" + value(summary, snapshot.max());
            list.add(text);
        }, longTaskTimer -> {
            ...
            String text = id(longTaskTimer) + ": active=" + value(longTaskTimer, activeTasks) + "; duration=" + time(baseTimeUnit, longTaskTimer.duration(baseTimeUnit)) + "; mean=" + time(baseTimeUnit, longTaskTimer.mean(baseTimeUnit)) + "; max=" + time(baseTimeUnit, longTaskTimer.max(baseTimeUnit));
            list.add(text);
        }, timeGauge -> {
            ...
            String text = id(functionTimer) + ": throughput=" + rate(functionTimer, step, count) + "; mean=" + time(baseTimeUnit, functionTimer.mean(baseTimeUnit));
            list.add(text);
    ...
    private static String id(final Meter meter) {
        ...
        String meterIdTags = id.getTags().stream().map(t -> Tag.of(namingConvention.tagKey(t.getKey()), namingConvention.tagValue(t.getValue()))).map(tag -> tag.getKey() + "=" + tag.getValue()).collect(Collectors.joining(",", "{", "}"));
